...
{
	public class StartedReceiving : MonoBehaviour
	{
		...
		*/
		...
		void OnTest2( OscMessage message )
		{
			...
			int frameCount;
			float time;
			float joystick_verticalValue;
			float joystick_horizentalValue;
			if(
				message.TryGet( 0, out frameCount ) && 
				message.TryGet( 1, out time ) &&
				message.TryGet( 2, out joystick_verticalValue) &&
				message.TryGet(3, out joystick_horizentalValue)
			) 
			{


				//Debug.Log( "Received test2\n" + frameCount + " " + time + " " + random + " " + joystick_horizentalValue + "\n" );
			...
			for( int i = 0; i < message.Count(); i++ )
			{
				...
				switch( argType )
				{
					case OscArgType.Float:
						...
						//TextScript.textFloat = floatValue;
